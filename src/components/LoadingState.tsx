...
export default function LoadingState({ progress }: LoadingStateProps) {
  const { current, total, cardName, originalImageUri, blingedImageUri, phase } = progress;
  ...
  return (
    <div className="bg-navy-dark border border-gray-800 rounded-xl p-6 space-y-5">
      ...
      <div className="space-y-2">
        <div className="flex justify-between text-sm">
          <span className="text-gray-400">
            {phase === "lookup" ? "Looking up cards..." : `Blinging card ${current} of ${total}`}
          </span>
        ...
        {cardName && (
          <p className="text-sm text-gray-300 truncate font-medium">{cardName}</p>
        )}
      </div>

      {/* Card images — show original → blinged */}
      {phase === "blinging" && (originalImageUri || blingedImageUri) && (
        <div className="flex items-center justify-center gap-4 md:gap-8">
          ...
          {/* Blinged card */}
          <div className="text-center space-y-2">
            <p className="text-xs text-gold uppercase tracking-wider font-semibold">Blinged</p>
            {blingedImageUri ? (
              <img
                src={blingedImageUri}
      ...
      <p className="text-xs text-gray-600 text-center leading-relaxed">
        ...
        To be respectful to the <a href="https://scryfall.com" target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-gray-400 underline">Scryfall API</a>, we space out requests — thanks for your patience!
      </p>
    </div>
  );
}
